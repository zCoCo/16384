classdef Robot3D
    ...
    properties (SetAccess = 'immutable')
        dof
        DH_params = struct(... % Denavit-Hartenberg Parameters for each Joint
            'as', [], ...
            'alphas', [], ...
            'ds', [] ...
        );
    ...
    methods
        ...
        function jacobians = jacobians(robot, thetas)
            % Returns the SE(3) Jacobian for each DH joint frame.
            if size(thetas, 2) ~= 1
                error('Expecting a column vector of joint angles.');
            end
            
            % Returns the vector of origin i
            function o = o(i)
                o = 
            end
        ...
        function js = js(r,ts), js = jacobians(r,ts); end % Shorthand for #jacobians
